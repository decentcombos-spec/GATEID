...
(function () {
  ...
  const KEYS = {
    ...
    tickets: 'gateid_v2_tickets',
    camEvents: 'gateid_v2_camevents'
  };
  ...
  function init() {
    ...
    if (!localStorage.getItem(KEYS.tickets)) save(KEYS.tickets, []);
    if (!localStorage.getItem(KEYS.camEvents)) save(KEYS.camEvents, []);
    ...
    state.tickets = load(KEYS.tickets, []);
    state.camEvents = load(KEYS.camEvents, []);
  ...
  function switchTab(name) {
    ...
    if (name === 'camera') renderCamEvents();
  ...
  function importJSON(file) {
    ...
    reader.onload = () => {
      try {
        ...
        state.camEvents = data.camEvents || [];
        ...
        save(KEYS.trucks, state.trucks);
        save(KEYS.logs, state.logs);
        save(KEYS.tickets, state.tickets);
        save(KEYS.camEvents, state.camEvents);
        save(KEYS.settings, state.settings);
  ...
  function renderTickets() {
    const statusF = $('#ticket-filter-status')?.value || 'all';
    const q = ($('#ticket-search')?.value || '').toLowerCase();
    let list = state.tickets.slice().sort((a, b) => (b.departedAt || '').localeCompare(a.departedAt || ''));
    if (statusF !== 'all') list = list.filter((t) => t.status === statusF);
    if (q) {
      list = list.filter((t) =>
        [t.ticketNo, t.plate, t.fromCode, t.toCode, t.material, t.company]
          .join(' ').toLowerCase().includes(q)
      );
    }
    ...
    if (!tbody) return;
    if (!list.length) {
      tbody.innerHTML = '<tr><td colspan="9" class="empty">No tickets yet. Create one when a truck leaves a loading site.</td></tr>';
    } else {
      tbody.innerHTML = list.map((t) => {
        const badge =
          t.status === 'open' ? '<span class="badge badge-open">In Transit</span>' :
          t.status === 'received' ? '<span class="badge badge-received">Received</span>' :
          '<span class="badge badge-cancelled">Cancelled</span>';
        const actions = [];
        if (t.status === 'open' && canEdit()) {
          actions.push('<button class="btn btn-primary btn-sm" data-receive-ticket="' + t.id + '">Accept</button>');
          actions.push('<button class="btn btn-ghost btn-sm" data-cancel-ticket="' + t.id + '">Cancel</button>');
        }
        if (t.status === 'open' || t.status === 'received') {
          actions.push('<button class="btn btn-ghost btn-sm" data-view-ticket="' + t.id + '">View</button>');
        }
        return '<tr>' +
          '<td><b>' + esc(t.ticketNo) + '</b></td>' +
          '<td class="plate-cell">' + esc(t.plate) + '</td>' +
          '<td>' + esc(t.fromCode) + '</td>' +
          '<td>' + esc(t.toCode) + '</td>' +
          '<td>' + esc(t.material) + '</td>' +
          '<td>' + esc((t.departedAt || '').replace('T', ' ').slice(0, 16)) + '</td>' +
          '<td>' + badge + '</td>' +
          '<td>' + (t.receivedAt ? esc(t.receivedAt.replace('T', ' ').slice(0, 16)) : '—') + '</td>' +
          '<td>' + actions.join(' ') + '</td></tr>';
      }).join('');
    }
    const c = $('#ticket-count');
    if (c) c.textContent = list.length + ' ticket' + (list.length !== 1 ? 's' : '');
  }

  function openCreateTicket() {
    if (!canEdit()) { toast('Cannot create ticket', true); return; }
    const fromSite = state.sites.find((s) => s.id === state.currentSiteId);
    const siteOpts = state.sites.map((s) =>
      '<option value="' + s.id + '"' + (s.id === state.currentSiteId ? ' selected' : '') + '>' + esc(s.code) + ' – ' + esc(s.name) + '</option>'
    ).join('');
    const toOpts = state.sites.map((s) =>
      '<option value="' + s.id + '">' + esc(s.code) + ' – ' + esc(s.name) + '</option>'
    ).join('');
    const truckOpts = state.trucks.map((t) =>
      '<option value="' + esc(t.plate) + '">' + esc(t.plate) + ' · ' + esc(t.unit || '') + ' · ' + esc(t.company || '') + '</option>'
    ).join('');
    $('#modal-title').textContent = 'Create Load Ticket';
    $('#modal-body').innerHTML =
      '<p class="hint">Ticket is created when the truck leaves the loading site. Destination accepts it on arrival.</p>' +
      '<div class="field"><label>License Plate *</label><input id="tk-plate" class="plate-input" list="tk-plate-dl" placeholder="ABC1234"><datalist id="tk-plate-dl">' + truckOpts + '</datalist></div>' +
      '<div class="field"><label>From (Loading Site) *</label><select id="tk-from">' + siteOpts + '</select></div>' +
      '<div class="field"><label>To (Dump / Receiving) *</label><select id="tk-to">' + toOpts + '</select></div>' +
      '<div class="field"><label>Material</label><input id="tk-material" list="mat-dl" placeholder="Material"></div>' +
      '<div class="field"><label>Company / Hauler</label><input id="tk-company" placeholder="Hauler"></div>' +
      '<div class="field"><label>Driver</label><input id="tk-driver" placeholder="Driver"></div>' +
      '<div class="field"><label>Truck / Unit</label><input id="tk-unit" placeholder="Unit #"></div>' +
      '<div class="field"><label>Est. Net (lbs)</label><input type="number" id="tk-net" placeholder="Optional"></div>' +
      '<div class="field"><label>Notes</label><input id="tk-notes" placeholder="Optional"></div>';
    $('#modal-foot').innerHTML =
      '<button class="btn btn-ghost" id="m-cancel">Cancel</button><button class="btn btn-primary" id="m-save">Create & Depart</button>';
    $('#modal').classList.remove('hidden');
    $('#m-cancel').onclick = closeModal;
    $('#tk-plate').addEventListener('change', () => {
      const p = normPlate($('#tk-plate').value);
      const tr = state.trucks.find((x) => normPlate(x.plate) === p);
      if (tr) {
        $('#tk-company').value = tr.company || '';
        $('#tk-driver').value = tr.driver || '';
        $('#tk-unit').value = tr.unit || '';
      }
    });
    $('#m-save').onclick = () => {
      const plate = normPlate($('#tk-plate').value);
      const fromId = $('#tk-from').value;
      const toId = $('#tk-to').value;
      if (!plate) { toast('Plate required', true); return; }
      if (!fromId || !toId) { toast('Select from and to sites', true); return; }
      if (fromId === toId) { toast('From and To must be different', true); return; }
      const from = state.sites.find((s) => s.id === fromId);
      const to = state.sites.find((s) => s.id === toId);
      const ticket = {
        id: uid(),
        ticketNo: nextTicketNo(),
        plate,
        fromId, fromCode: from?.code || '', fromName: from?.name || '',
        toId, toCode: to?.code || '', toName: to?.name || '',
        material: $('#tk-material').value.trim(),
        company: $('#tk-company').value.trim(),
        driver: $('#tk-driver').value.trim(),
        unit: $('#tk-unit').value.trim(),
        net: parseFloat($('#tk-net').value) || 0,
        notes: $('#tk-notes').value.trim(),
        status: 'open',
        departedAt: new Date().toISOString(),
        departedBy: state.user.username,
        receivedAt: null,
        receivedBy: null,
        history: [{ action: 'created', by: state.user.username, at: new Date().toISOString() }]
      };
      ...
      // Also log OUT at loading site
      const logEntry = {
        id: uid(),
        date: today(), time: nowTime(), timestamp: new Date().toISOString(),
        siteId: fromId, siteName: from?.name || '', siteCode: from?.code || '',
        plate, direction: 'OUT',
        truck: ticket.unit, company: ticket.company, driver: ticket.driver,
        material: ticket.material, ticket: ticket.ticketNo,
        gross: 0, tare: 0, net: ticket.net,
        notes: 'Ticket ' + ticket.ticketNo + ' departed → ' + ticket.toCode,
        photo: null, known: isKnownPlate(plate),
        loggedBy: state.user.username, loggedByName: state.user.name,
        history: [{ action: 'created', by: state.user.username, at: new Date().toISOString() }]
      };
      ...
      renderRecent();
      renderDash();
      toast('Ticket ' + ticket.ticketNo + ' created · departed ' + ticket.fromCode);
    };
  }

  function receiveTicket(id) {
    if (!canEdit()) { toast('Cannot receive', true); return; }
    const t = state.tickets.find((x) => x.id === id);
    if (!t || t.status !== 'open') return;
    $('#modal-title').textContent = 'Accept Ticket ' + t.ticketNo;
    $('#modal-body').innerHTML =
      '<p class="hint">Confirm receipt at <b>' + esc(t.toCode) + '</b></p>' +
      '<div class="field"><label>Plate</label><input class="plate-input" value="' + esc(t.plate) + '" readonly></div>' +
      '<div class="field"><label>From → To</label><input value="' + esc(t.fromCode + ' → ' + t.toCode) + '" readonly></div>' +
      '<div class="field"><label>Material</label><input value="' + esc(t.material) + '" readonly></div>' +
      '<div class="field"><label>Gross (lbs)</label><input type="number" id="rc-gross" placeholder="Optional"></div>' +
      '<div class="field"><label>Tare (lbs)</label><input type="number" id="rc-tare" placeholder="Optional"></div>' +
      '<div class="field"><label>Notes</label><input id="rc-notes" placeholder="Optional receive notes"></div>';
    $('#modal-foot').innerHTML =
      '<button class="btn btn-ghost" id="m-cancel">Cancel</button><button class="btn btn-primary" id="m-save">Confirm Received</button>';
    $('#modal').classList.remove('hidden');
    $('#m-cancel').onclick = closeModal;
    $('#m-save').onclick = () => {
      const g = parseFloat($('#rc-gross').value) || 0;
      const tare = parseFloat($('#rc-tare').value) || 0;
      const net = g > 0 ? Math.max(0, g - tare) : t.net;
      t.status = 'received';
      t.receivedAt = new Date().toISOString();
      t.receivedBy = state.user.username;
      t.receivedGross = g;
      t.receivedTare = tare;
      t.receivedNet = net;
      t.receiveNotes = $('#rc-notes').value.trim();
      t.history = t.history || [];
      t.history.push({ action: 'received', by: state.user.username, at: t.receivedAt });
      save(KEYS.tickets, state.tickets);

      const to = state.sites.find((s) => s.id === t.toId);
      const logEntry = {
        id: uid(),
        date: today(), time: nowTime(), timestamp: new Date().toISOString(),
        siteId: t.toId, siteName: to?.name || t.toName, siteCode: t.toCode,
        plate: t.plate, direction: 'IN',
        truck: t.unit, company: t.company, driver: t.driver,
        material: t.material, ticket: t.ticketNo,
        gross: g, tare: tare, net: net,
        notes: 'Ticket ' + t.ticketNo + ' received from ' + t.fromCode + (t.receiveNotes ? ' · ' + t.receiveNotes : ''),
        photo: null, known: isKnownPlate(t.plate),
        loggedBy: state.user.username, loggedByName: state.user.name,
        history: [{ action: 'created', by: state.user.username, at: new Date().toISOString() }]
      };
      state.logs.unshift(logEntry);
      ...
      renderRecent();
      renderDash();
      toast('Ticket ' + t.ticketNo + ' received at ' + t.toCode);
    };
  }
  ...
  function cancelTicket(id) {
    ...
    const t = state.tickets.find((x) => x.id === id);
    if (!t || t.status !== 'open') return;
    if (!confirm('Cancel ticket ' + t.ticketNo + '?')) return;
    t.status = 'cancelled';
    t.history = t.history || [];
    t.history.push({ action: 'cancelled', by: state.user.username, at: new Date().toISOString() });
    save(KEYS.tickets, state.tickets);
    renderTickets();
  ...
  function viewTicket(id) {
    const t = state.tickets.find((x) => x.id === id);
    if (!t) return;
    $('#modal-title').textContent = 'Ticket ' + t.ticketNo;
    $('#modal-body').innerHTML =
      '<div class="field"><label>Status</label><div>' + esc(t.status) + '</div></div>' +
      '<div class="field"><label>Plate</label><div class="plate-cell">' + esc(t.plate) + '</div></div>' +
      '<div class="field"><label>Route</label><div>' + esc(t.fromCode) + ' → ' + esc(t.toCode) + '</div></div>' +
      '<div class="field"><label>Material</label><div>' + esc(t.material || '—') + '</div></div>' +
      '<div class="field"><label>Company / Driver</label><div>' + esc(t.company || '—') + ' / ' + esc(t.driver || '—') + '</div></div>' +
      '<div class="field"><label>Departed</label><div>' + esc(t.departedAt) + ' by ' + esc(t.departedBy) + '</div></div>' +
      '<div class="field"><label>Received</label><div>' + (t.receivedAt ? esc(t.receivedAt) + ' by ' + esc(t.receivedBy) : '—') + '</div></div>' +
      '<div class="field"><label>Net</label><div>' + (t.receivedNet || t.net || '—') + ' lbs</div></div>' +
      '<div class="field"><label>Notes</label><div>' + esc(t.notes || '—') + '</div></div>';
    $('#modal-foot').innerHTML = '<button class="btn btn-primary" id="m-cancel">Close</button>';
    $('#modal').classList.remove('hidden');
    $('#m-cancel').onclick = closeModal;
  }

  /* ========== CAMERA WEBHOOK / EVENTS ========== */
  function renderCamEvents() {
    const el = $('#cam-events-list');
    if (!el) return;
    if (!state.camEvents.length) {
      el.innerHTML = '<div class="empty">No camera events yet. Use the Test Injector or connect your Reolink middleman.</div>';
      return;
    }
    el.innerHTML = state.camEvents.slice(0, 30).map((e) =>
      '<div class="feed-item"><span><span class="plate">' + esc(e.plate) + '</span> · ' +
      esc(e.direction || 'IN') + ' · ' + esc(e.siteCode || '') +
      (e.source ? ' · ' + esc(e.source) : '') +
      '</span><span>' + esc((e.at || '').replace('T', ' ').slice(0, 19)) + '</span></div>'
    ).join('');
  }

  function ingestCameraEvent(payload) {
    if (!payload || !payload.plate) throw new Error('plate is required');
    const plate = normPlate(payload.plate);
    if (plate.length < 3) throw new Error('invalid plate');

    let site = null;
    if (payload.siteCode) {
      site = state.sites.find((s) => s.code.toUpperCase() === String(payload.siteCode).toUpperCase());
    }
    if (!site && payload.siteId) site = state.sites.find((s) => s.id === payload.siteId);
    if (!site) site = state.sites.find((s) => s.id === state.currentSiteId) || state.sites[0];

    const direction = (payload.direction || 'IN').toUpperCase() === 'OUT' ? 'OUT' : 'IN';
    const matched = state.trucks.find((t) => normPlate(t.plate) === plate);
    ...
    const entry = {
      ...
      plate,
      direction,
      truck: matched?.unit || payload.truck || '',
      company: matched?.company || payload.company || '',
      driver: matched?.driver || payload.driver || '',
      material: payload.material || '',
      ticket: payload.ticket || '',
      gross: payload.gross || 0,
      tare: payload.tare || matched?.tare || 0,
      net: payload.net || 0,
      notes: 'Camera: ' + (payload.source || 'webhook') + (payload.confidence ? ' · conf ' + payload.confidence : ''),
      photo: payload.photo || null,
      known: isKnownPlate(plate),
      loggedBy: 'camera',
      loggedByName: payload.source || 'Camera',
      history: [{ action: 'created', by: 'camera', at: new Date().toISOString() }]
    };

    state.logs.unshift(entry);
    if (state.logs.length > 300) {
      state.logs.slice(200).forEach((l) => { if (l.photo) l.photo = null; });
    }
    save(KEYS.logs, state.logs);

    state.camEvents.unshift({
      id: entry.id,
      plate,
      direction,
    ...
    if (state.camEvents.length > 100) state.camEvents = state.camEvents.slice(0, 100);
    save(KEYS.camEvents, state.camEvents);

    // Switch active site if event came from another gate
    if (site && site.id !== state.currentSiteId) {
  ...
  function injectCameraEvent() {
    let payload = null;
    const raw = ($('#inj-json')?.value || '').trim();
    if (raw) {
      try { payload = JSON.parse(raw); }
      catch (err) { toast('Invalid JSON: ' + err.message, true); return; }
    } else {
      payload = {
        plate: $('#inj-plate').value,
        direction: $('#inj-dir').value,
        siteCode: $('#inj-site').value || undefined,
        source: $('#inj-source').value || 'reolink-test'
      };
    }
    try {
      const entry = ingestCameraEvent(payload);
      toast('Camera event logged: ' + entry.plate + (entry.known ? '' : ' ⚠ unknown'));
      $('#inj-plate').value = '';
      $('#inj-json').value = '';
    } catch (err) {
  ...
  // Expose for middleman / console testing when same origin
  window.GATEID = window.GATEID || {};
  window.GATEID.ingestCameraEvent = function (payload) {
    try {
      const entry = ingestCameraEvent(payload);
      return { ok: true, id: entry.id, plate: entry.plate };
    } catch (err) {
  ...
  function bind() {
    ...
    const btnCreateTicket = $('#btn-create-ticket');
    if (btnCreateTicket) btnCreateTicket.addEventListener('click', openCreateTicket);
    const btnInject = $('#btn-inject-event');
    if (btnInject) btnInject.addEventListener('click', injectCameraEvent);
    const btnClearCam = $('#btn-clear-cam-events');
    if (btnClearCam) btnClearCam.addEventListener('click', () => {
      state.camEvents = [];
      save(KEYS.camEvents, state.camEvents);
      renderCamEvents();
      toast('Camera events cleared');
    });
    const tkFilter = $('#ticket-filter-status');
    if (tkFilter) tkFilter.addEventListener('change', renderTickets);
    const tkSearch = $('#ticket-search');
    if (tkSearch) tkSearch.addEventListener('input', renderTickets);
    ...
    document.addEventListener('click', (e) => {
      ...
      const recvTk = e.target.closest('[data-receive-ticket]');
      if (recvTk) receiveTicket(recvTk.dataset.receiveTicket);
      const cancelTk = e.target.closest('[data-cancel-ticket]');
      if (cancelTk) cancelTicket(cancelTk.dataset.cancelTicket);
      const viewTk = e.target.closest('[data-view-ticket]');
      if (viewTk) viewTicket(viewTk.dataset.viewTicket);
